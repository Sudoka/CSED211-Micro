//CHANGE SOURCE FOR LTS280Q1-PE LCD AND BACKLIGHT
...
void Port_Init(void)
{
    ...
    rGPBDAT|=(1<<1);	
    rGPBCON&=0x3ffff3;
    rGPBCON|=(1<<2);	// Output(nIrDATXDEN) disable
    rGPBUP|=(1<<1);	//Uart port pull-up disable

    rGPBCON&=0x3ffffc; //pwm input temp. lcd backlight
...
//========================**[ BOARD LED ]=================================
void Led_Display(int data) //20031030 doong2
{
          //Active is low.(LED On)
          // GPF7  GPF6   GPF5   GPF4
          //nLED_8 nLED4 nLED_2 nLED_1
//    rGPFDAT = (rGPFDAT & 0xf) | !((data & 0xf)<<4);
//    rGPFDAT = (rGPFDAT & ~(0xf<<4)) | ((~data & 0xf)<<4);    //smdk2410

	if(data ==0 )
	rGPADAT |= 0x00200000; //STATUS_LED off
	else
	rGPADAT &= 0x005fffff; //STATUS_LED on
}
